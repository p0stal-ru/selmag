...
class ProductsRestControllerIT {
    ...
    void findProducts_ReturnsProductList() throws Exception {
        ...
        this.mockMvc.perform(requestBuilder)
                ...
                .andDo(print())
                .andExpectAll(
                        status().isOk(), content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON),
                        content().json("""
                                [
    ...
    void createProduct_RequestIsValid_ReturnsNewProduct() throws Exception {
        ...
        var requestBuilder = MockMvcRequestBuilders.post("/catalogue-api/products")
                ...
                .content("""
                        {"title": "Еще один новый товар", "details": "Описание"}""")
        ...
        mockMvc.perform(requestBuilder)
                ...
                .andDo(print())
                .andExpectAll(
                        status().isCreated(), header().string(HttpHeaders.LOCATION, "http://localhost/catalogue-api/products/1"),
                        content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON),
                        content().json("""
                                {"id":  1,"title": "Еще один новый товар", "details": "Описание"}""")
                );
    }
